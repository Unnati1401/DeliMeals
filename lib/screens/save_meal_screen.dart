...
class _SaveMealState extends State<SaveMeal> {
  ...
  List<Widget> listOfTextFormFields(
      int numberOfSteps, int numberOfIngredients, final routeArgs) {
    List<Widget> list = [];
    ...
    for (int i = 1; i <= numberOfIngredients; i++) {
      ...
      list.add(TextFormField(
        ...
        validator: (value) {
          if (value.isEmpty) return 'Please enter the ingredient.';
          return null;
        },
      ));
    }
    ...
    for (int i = 1; i <= numberOfSteps; i++) {
      ...
      list.add(TextFormField(
        ...
        validator: (value) {
          if (value.isEmpty) return 'Please enter the step.';
          return null;
        },
      ));
    }
  ...
  Future<void> _submitForm(
      final routeArgs, int numberOfSteps, int numberOfIngredients) async {
    ...
    Meal meal = Meal(
      ...
      complexity: Complexity.values.firstWhere(
          (e) => e.toString() == 'Complexity.' + routeArgs['complexity']),
      affordability: Affordability.values.firstWhere(
          (e) => e.toString() == 'Affordability.' + routeArgs['affordability']),
      ...
      body: _isLoading
          ...
          : Padding(
              ...
              child: Form(
                key: _form,
                child: ListView(
                  children: listOfTextFormFields(routeArgs['numberOfSteps'],
                      routeArgs['numberOfIngredients'], routeArgs),
                ),
              ),
            ),
    );
