...
class Meals with ChangeNotifier {
  ...
  Future<void> toggleFavoriteStatus(
    ...
    final url =
        'https://mealsapp-8267e.firebaseio.com/userFavorites/$userId/$id.json?auth=$token';
  ...
  Future<void> addMeal(Meal meal) async {
    List<String> rev = [""];
    final url =
        'https://mealsapp-8267e.firebaseio.com/meals.json?auth=$authToken';
    try {
  ...
  Future<void> fetchMeals([bool filterByUser = false]) async {
    ...
    var url =
        'https://mealsapp-8267e.firebaseio.com/meals.json?auth=$authToken&$filterString';

    var url2 =
        'https://mealsapp-8267e.firebaseio.com/userFilters/$userId.json?auth=$authToken';
    ...
    try {
      ...
      url =
          'https://mealsapp-8267e.firebaseio.com/userFavorites/$userId.json?auth=$authToken';
          ...
          loadedMeals.add(
            Meal(
              ...
              affordability: Affordability.values.firstWhere(
                  (e) => e.toString() == mealData['affordability'],
                  orElse: () => null),
              complexity: Complexity.values.firstWhere(
                  (e) => e.toString() == mealData['complexity'],
                  orElse: () => null),
      ...
      //adds meals to _mealsList according to filters
        ...
        extractedData.forEach((mealId, mealData) {
          ...
          loadedMeals.removeWhere((element) =>
              (extractedData2['isGlutenFree'] == true &&
                      element.isGlutenFree == false ||
                  extractedData2['isLactoseFree'] == true &&
                      element.isLactoseFree == false ||
                  extractedData2['isVegan'] == true &&
                      element.isVegan == false ||
                  extractedData2['isVegetarian'] == true &&
    ...
    _mealsList.forEach((element) {
      print(element.title);
      print(element.numberOfLikes);
    });
  }

  Future<void> deleteMeal(String id) async {
    final url =
        'https://mealsapp-8267e.firebaseio.com/meals/$id.json?auth=$authToken';
  ...
  Future<void> setMealsByFilters() async {
    ...
    try {
      ...
      if (extractedData == null) return;
      print(extractedData);
      _mealsList.removeWhere((element) =>
          (extractedData['isGlutenFree'] == true &&
                  element.isGlutenFree == false ||
              extractedData['isLactoseFree'] == true &&
                  element.isLactoseFree == false ||
              extractedData['isVegan'] == true && element.isVegan == false ||
              extractedData['isVegetarian'] == true &&
                  element.isVegetarian == false));
      notifyListeners();
    } catch (error) {
      throw error;
  ...
  Future<void> like(String id) async {
    final url =
        'https://mealsapp-8267e.firebaseio.com/meals/$id/numberOfLikes.json?auth=$authToken';
    final mealIndex = _mealsList.indexWhere((element) => element.id == id);
    print(mealIndex);
    Meal meal = _mealsList[mealIndex];
  ...
  Future<void> dislike(String id) async {
    final url =
        'https://mealsapp-8267e.firebaseio.com/meals/$id/numberOfDislikes.json?auth=$authToken';
    final mealIndex = _mealsList.indexWhere((element) => element.id == id);
    print(mealIndex);
    Meal meal = _mealsList[mealIndex];
  ...
  Future<void> addReview(String id, String reviewToBeAdded) async {
    final url =
        'https://mealsapp-8267e.firebaseio.com/meals/$id/reviews.json?auth=$authToken';
    final mealIndex = _mealsList.indexWhere((element) => element.id == id);
    print("mealIndex: $mealIndex");
    Meal meal = _mealsList[mealIndex];
    meal.reviews.add(reviewToBeAdded);
    print("reviews: $meal.reviews");
    _mealsList[mealIndex] = meal;
    print(meal.reviews.runtimeType);
    try {
      final response = await http.put(
        url,
        body: json.encode(meal.reviews),
      );
      if (response.statusCode >= 400) {
        meal.reviews.removeWhere((element) => element == reviewToBeAdded);
        _mealsList[mealIndex] = meal;
      }
    } catch (error) {
      meal.reviews.removeWhere((element) => element == reviewToBeAdded);
